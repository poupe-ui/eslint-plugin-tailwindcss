...
- **Missing @reference**: Adds `@reference "<fallbackReference>";` at the beginning
  of the style block (defaults to `"tailwindcss"`, configurable via options)

## Known Limitations

- The auto-fix inserts `@reference` before the first child node. If the
  style block starts with `@charset`, the fix will incorrectly place
  `@reference` before it. In practice, `@charset` is not used in Vue
  SFC `<style>` blocks.
